...
![Архитектура приложения](uml-mum.png)
...
![Архитектура приложения](diagram.png)
...
5 Тестирование
Unit-тестирование – это проверка отдельных единиц кода (функций, методов, классов) в изоляции от внешних зависимостей (БД, сеть, файловая система). В юнит-тестах используют моки и заглушки, чтобы заменить реальные зависимости – так тестируется только внутренняя логика. Юнит-тесты быстрые, детальные и ориентированы на локализацию ошибок.
Интеграционное тестирование – проверка взаимодействия нескольких модулей/слоёв приложения между собой. Цель –  убедиться, что модули правильно интегрированы и данные корректно проходят через слои (Controller → Service → Repository). Интеграционные тесты обычно медленнее и охватывают больше кода, но дают уверенность в «сработке» системы как целого.
Процесс тестирования: 
1)	Подготовка кода: реализованы классы Product, ProductRepository, ProductService, ProductController. 
2)	Юнит-тесты: тестировали ProductService. Проверяли: корректную валидацию, вызовы репозитория, возвраты булевых значений и обработку несуществующих записей.
3)	Интеграционные тесты: создавались реальные экземпляры Repository+Service+Controller и выполнялись сценарии, покрывающие полный поток: добавление → получение → удаление → повторное добавление.
4)	Запуск тестов: тесты запускаются через Maven или Gradle (gradle test). Юнит-тесты выполняются быстро; интеграционные тесты немного дольше.
5)	Анализ результатов: для каждого теста проверяли ассёрты. Все приведённые тесты на основе кода – успешны. В отчете фиксируем пройденные тесты и их результаты.
Тестируем ProductService:
...
public class ProductServiceUnitTest {
    ...
    // 1. Успешное добавление продукта
    @Test
    void addProduct_validData_returnsTrue() {
        when(repo.add(any())).thenReturn(true);

        boolean result = service.addProduct(1, "Laptop", 1200.0);

        assertTrue(result);
        verify(repo, times(1)).add(any());
    }

    // 2. Некорректная цена → false, repo не вызывается
    @Test
    void addProduct_negativePrice_returnsFalse() {
        boolean result = service.addProduct(2, "Phone", -300.0);

        assertFalse(result);
        verify(repo, never()).add(any());
    }

    // 3. Продукт не найден → null
    @Test
    void getProduct_notFound_returnsNull() {
        when(repo.getById(999)).thenReturn(null);

        Product product = service.getProduct(999);
...
Юнит-тест 1 – Добавление продукта с корректными данными
Цель теста: Проверить, что метод addProduct() корректно обрабатывает валидные входные данные.
Что проверяется:
•	название не пустое
•	цена положительная
•	метод репозитория вызывается
•	результат true
Ожидаемый результат:Товар успешно добавляется, и сервис возвращает true.
Фактический результат: Товар успешно добавился, сервис возвращает true.
	Вывод: Бизнес-логика сервиса работает корректно при нормальных условиях.
Юнит-тест 2 – Добавление продукта с отрицательной ценой
Цель теста:Убедиться, что сервис отклоняет некорректные данные.
Что проверяется:
•	цена ≤ 0
•	сервис возвращает false
Ожидаемый результат:Сервис вернёт false, и никакого добавления не произойдёт.
Фактический результат:Сервис возвращает false, и никакого добавления не происходит.
Вывод: Механизм валидации данных корректно защищает систему от невалидных значений.
Юнит-тест 3 – Получение несуществующего продукта
Цель теста: Проверить поведение сервиса при запросе товара, которого нет в базе.
Что проверяется:
•	вызов repo.getById()
•	корректное возврат null при отсутствии объекта
Ожидаемый результат: Метод вернёт null.
Фактический результат: Метод вернул null.
Вывод: Сервис корректно обрабатывает ситуацию, когда товар не найден.
Вывод по Unit-тестированию:
Три юнит-теста показали, что:
•	валидация данных работает верно;
•	сервис корректно взаимодействует с репозиторием;
•	ошибки данных отсекаются до обращения к базе;
•	работа с несуществующими объектами реализована правильно.
Юнит-тесты подтвердили корректность бизнес-логики сервиса.
Интеграционное тестирование:
Тестируем связку: Controller → Service → Repository
...
Интеграционный тест 1 – Полный жизненный цикл продукта
Цель теста: Проверить, как корректно вместе работают три слоя:
...
Фактический результат: Все шаги выполняются без ошибок.
Вывод: Связка всех компонентов системы работает как единое целое.

Интеграционный тест 2 – Доступность товара сразу после добавления
Цель теста: Убедиться, что данные мгновенно доступны во всех слоях.
Сценарий:
•	Добавить товар
•	Сразу получить товар
Ожидаемый результат: Полученный товар совпадает с добавленным.
Фактический результат: Полученный товар совпадает с добавленным.
Вывод: Асинхронность отсутствует, данные хранятся и отдаются корректно.
Интеграционный тест 3 – Удаление и повторное добавление с тем же ID
Цель теста: Проверить сброс состояния хранилища.
Сценарий:
•	Добавить товар
•	Удалить товар
•	Добавить новый товар с тем же ID
•	Проверить обновлённые данные
Ожидаемый результат: После удаления повторное добавление проходит успешно, а данные обновляются.
Фактический результат: После удаления повторное добавление проходит успешно, а данные обновляются.
Вывод: Система корректно обрабатывает повторные операции и изменения состояния.
Вывод по интеграционному тестированию:
Интеграционные тесты доказали, что:
•	все слои приложения работают согласованно;
•	данные корректно проходят через контроллер, сервис и репозиторий;
•	операции добавления, получения и удаления стабильны;
•	состояние хранилища корректно обновляется.

6 Документация
6.1 Общие сведения

Программное средство «Онлайн-магазин товаров для мам и детей» предоставляет REST API для продажи товаров для детей и мам. API реализован на серверной части с использованием Java Spring Boot. Все запросы и ответы передаются в формате JSON. Аутентификация и авторизация пользователей обеспечивается через Spring Security.
Роли пользователей:
	Administrator – управление товарами,  управление продавцами, управление клиентами, управление аналитикой, управление магазинами;
	Saler – управление магазином, управление товарами;
	Customer – управление корзиной.

6.2 Основные разделы API:
...

{
  ...
  "description": "Коляски для новорожденных и детей до 3 лет"
}
5 Заявки на кредит	
GET /products – список товаров (с фильтрацией: категория, цена, бренд)
POST /products – добавить товар
GET /products/{productId} – информация о товаре
PUT /products/{productId} – обновление товара
...

{
  "productId": 1,
  "categoryId": 1,
  "name": "Коляска BabyLux Comfort",
  "price": 450.00,
  "brand": "BabyLux",
  "stock": 12,
  "description": "Удобная коляска для детей до 3 лет",
  "createdAt": "2025-09-01T14:00:00"
}

6 Корзина
GET /cart/{customerId} – получить корзину
POST /cart/{customerId}/items – добавить товар
PUT /cart/{customerId}/items/{itemId} – изменить количество
DELETE /cart/{customerId}/items/{itemId} – удалить товар
Пример позиции в корзине:
...
7 Заказы
GET /orders – список заказов (фильтр: клиент, дата, статус)
POST /orders – создать заказ
GET /orders/{orderId}
PUT /orders/{orderId} – изменить статус
DELETE /orders/{orderId} – отменить заказ
...
В репозитории положен файл спецификации: swagger.yaml. Этот файл соответствует OpenAPI 3.0 и может быть импортирован в Swagger UI, Redoc или Postman.  На рисунке 1 показан Swagger UI с описанием методов API. 

<img width="436" height="224" alt="image" src="https://github.com/user-attachments/assets/6ea03742-10f4-48e2-9bc0-1826e2b27a15" />

 
Рисунок 1 – Swagger UI

На рисунке 2 представлен пример запроса в Swagger UI.

 <img width="417" height="266" alt="image" src="https://github.com/user-attachments/assets/71105c0a-9468-4a4e-a6e4-436d560e4462" />


Рисунок 2 – Пример запроса в Swagger UI

На рисунке 3 показан импорт файла спецификации в Postman с примером запроса к API.  

 <img width="430" height="235" alt="image" src="https://github.com/user-attachments/assets/72283855-e175-456b-8007-91416b92734f" />


Рисунок 3 – API в Postman

7 Оценка качества кода

Для оценки качества кода программного средства «Онлайн магазин товаров для мам и детей» были использованы следующие метрики и инструменты:
1 Покрытие кода тестами 
– общий процент покрытия: 80%  
– покрытие классов: 82%  
– покрытие методов: 81%  
...
– количество предупреждений: 8 (незначительные: стиль кода, лишние импорты)  
– дублирование кода: 2%  
– нарушения правил кодирования: отсутствуют критические, 1 средняя
...
Тестирование проводилось с использованием JUnit 5. Unit-тесты проверяют правильность работы ключевых сервисов и отдельных компонентов, а интеграционные тесты подтверждают корректное взаимодействие модулей системы и работу API.
В целом результаты анализа качества кода показывают, что разработанное программное обеспечение отвечает современным требованиям надежности и удобства сопровождения.
